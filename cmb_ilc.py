...
class CMBILC(object):
    ...
    def __init__(self, Nu, Beam, Noise, cl=None, name="test", fg=True, atm=False, lMaxT=3500., lMaxP=3500.):
        '''Simple ILC class, following Delabrouille Cardoso 2007.
      ...
      '''
        ...
        for i in range(self.nNu):
            for j in range(self.nNu):
                ...
                self.cmb[i, j] = CMB(beam=beam, noise=noise, nu1=self.Nu[i], nu2=self.Nu[j], lMin=self.lMin,
                                     lMaxT=self.lMaxT, lMaxP=self.lMaxP, fg=fg, atm=atm, name=None)
    ...
    def weightsIlcCmbEE(self, l):
        '''CMB ILC: min var unbiased CMB estimator, from linear combination of freq E-mode maps.
      Assumes the various freq channels are already calibrated to unit response to CMB unit.
      The weights include the covariance between freq channels.
      They add up to 1.
      '''
    ...
    def plotWeightsIlcCmb(self):
        ...
        W = np.array(list(map(self.weightsIlcCmb, L)))

        fig = plt.figure(0)
        ax = fig.add_subplot(111)
        #
        for i in range(self.nNu):
            ax.plot(L, W[:, i], label=str(int(self.Nu[i] / 1.e9)) + ' GHz')
        #
    ...
    def plotPowerIlcCmb(self):
        ...
        powerIlcCmb = np.array(list(map(f, L)))
        ...
        # individual channels
        for i in range(self.nNu):
            cl = np.array(list(map(self.cmb[i, i].ftotalTT, L)))
            ax.plot(L, factor * cl, '--', label=str(int(self.Nu[i] / 1.e9)) + ' GHz')
        #
        # ILC
        ax.plot(L, factor * powerIlcCmb, 'k', label=r'ILC')
        # lensed CMB
        cl = np.array(list(map(self.cmb[i, i].flensedTT, L)))
        ...
        for i in range(self.nNu):
            cl = np.array(list(map(self.cmb[i, i].ftotalTT, L)))
            ax.plot(L, cl / powerIlcCmb, '--', label=str(int(self.Nu[i] / 1.e9)) + ' GHz')
        #
        # ILC
    ...
    def plotWeightsIlcTsz(self):
        nL = 501
        L = np.logspace(np.log10(self.lMin), np.log10(self.lMaxT), nL, 10.)
        W = np.array(list(map(self.weightsIlcTsz, L)))
    ...
    def plotPowerIlcTsz(self):
        ...
        powerIlcTsz = np.array(list(map(f, L)))
        ...
        # tSZ
        cl = np.array(list(map(self.cmb[0, 0].ftSZ, L)))
        ...
        for i in range(self.nNu):
            cl = np.array(list(map(self.cmb[i, i].ftotalTT, L)))
            cl /= self.cmb[0, 0].tszFreqDpdceT(self.Nu[i]) ** 2
            ax.plot(L, cl / powerIlcTsz, '--', label=str(int(self.Nu[i] / 1.e9)) + ' GHz')
        #
        # ILC
        ax.plot(L, np.ones_like(L), 'k', label=r'ILC')
        #
    ...
    def weightsConstrainedIlcTszNoCib(self, l):
      ...
      '''
        ...
        for i in range(self.nNu):
            for j in range(self.nNu):
                c[i, j] = self.cmb[i, j].flensedTT(l) + self.cmb[i, j].fkSZ(l) + self.cmb[i, j].fradioPoisson(l) + \
                          self.cmb[i, j].fdetectorNoise(l) + self.cmb[i, j].fCIB(l)
                c[i, j] /= self.cmb[0, 0].tszFreqDpdceT(self.Nu[i]) * self.cmb[0, 0].tszFreqDpdceT(self.Nu[j])
        # invert matrix
        cInv = np.linalg.inv(c)
        # freq dpdce of the CIB to be nulled
        f = np.array(list(map(self.cmb[0, 0].cibPoissonFreqDpdceT, self.Nu)))
        f /= np.array(list(map(self.cmb[0, 0].tszFreqDpdceT, self.Nu)))
        # useful numbers
        S0 = np.sum(cInv)
        ...
        w /= S0 * S2 - S1 ** 2
        w /= np.array(list(map(self.cmb[0, 0].tszFreqDpdceT, self.Nu)))
        return w

    def plotWeightsConstrainedIlcTszNoCib(self):
        nL = 501
        L = np.logspace(np.log10(self.lMin), np.log10(self.lMaxT), nL, 10.)
        W = np.array(list(map(self.weightsConstrainedIlcTszNoCib, L)))
    ...
    def plotPowerConstrainedIlcTszNoCib(self):
        ...
        powerConstrainedIlcTszNoCib = np.array(list(map(f, L)))
        ...
        for i in range(self.nNu):
            cl = np.array(list(map(self.cmb[i, i].ftotalTT, L)))
            cl /= self.cmb[0, 0].tszFreqDpdceT(self.Nu[i]) ** 2
            ax.plot(L, cl / powerConstrainedIlcTszNoCib, '--', label=str(int(self.Nu[i] / 1.e9)) + ' GHz')
        #
        # ILC
        ax.plot(L, np.ones_like(L), 'k', label=r'ILC')
        #
