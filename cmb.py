...
class CMB(object):
    ...
    def __init__(self, beam=1., noise=1., nu1=150.e9, nu2=150.e9, lMin=2., lMaxT=3.e3, lMaxP=5.e3, fg=True, atm=False,
                 name=None, data_dir='/Users/antonbaleatolizancos/Software/BasicILC'):

        # name
        self.name = "cmb_beam" + str(round(beam, 3)) + "_noise" + str(round(noise, 3)) + "_nu" + str(
            int(nu1 / 1.e9)) + "_nu" + str(int(nu2 / 1.e9)) + "_lmin" + str(int(lMin)) + "_lmaxT" + str(
            int(lMaxT)) + "_lmaxP" + str(int(lMaxP))
        if atm:
            self.name += "_atmnoise"
        if name is not None:
            self.name += "_" + name
        ...
        # TT
        if fg and not atm:
            self.ftotalTT = lambda l: self.flensedTT(l) + self.fkSZ(l) + self.fCIB(l) + self.ftSZ(l) + self.ftSZ_CIB(
                l) + self.fradioPoisson(l) + self.fgalacticDust(l) + self.fdetectorNoise(l)
        elif fg and atm:
            self.ftotalTT = lambda l: self.flensedTT(l) + self.fkSZ(l) + self.fCIB(l) + self.ftSZ(l) + self.ftSZ_CIB(
                l) + self.fradioPoisson(l) + self.fgalacticDust(l) + self.fdetectorNoise(l) + self.fatmosphericNoiseTT(
                l)
        ...
        self.fkSZ = lambda l: a_kSZ * fkSZ_template(l) * self.kszFreqDpdceT(self.nu1) * self.kszFreqDpdceT(
            self.nu2) / self.kszFreqDpdceT(self.nu0) ** 2
        ...
        a_tSZ = 4.0
        self.ftSZ = lambda l: a_tSZ * ftSZ_template(l) * self.tszFreqDpdceT(self.nu1) * self.tszFreqDpdceT(
            self.nu2) / self.tszFreqDpdceT(self.nu0) ** 2
        ...
        self.ftSZ_CIB = lambda l: -2. * xi * np.sqrt(a_tSZ * a_CIBC) * ftSZCIB_template(l) * (
                    self.tszFreqDpdceT(self.nu1) * self.cibPoissonFreqDpdceT(self.nu2) + self.tszFreqDpdceT(
                self.nu2) * self.cibPoissonFreqDpdceT(self.nu1)) / (
                                              2. * self.tszFreqDpdceT(self.nu0) * self.cibPoissonFreqDpdceT(self.nu0))
    ...
    def fCIBPoisson(self, l):
      ...
      '''
        ...
        result *= self.cibPoissonFreqDpdceT(self.nu1) * self.cibPoissonFreqDpdceT(self.nu2) / self.cibPoissonFreqDpdceT(
            self.nu0) ** 2
        return result
    ...
    def fCIBClustered(self, l, nu1=None, nu2=None):
      ...
      '''
        ...
        result *= self.cibClusteredFreqDpdceT(self.nu1) * self.cibClusteredFreqDpdceT(
            self.nu2) / self.cibClusteredFreqDpdceT(self.nu0) ** 2
        return result
    ...
    def fradioPoisson(self, l):
      ...
      '''
        ...
        result *= self.radioPoissonFreqDpdceT(self.nu1) * self.radioPoissonFreqDpdceT(
            self.nu2) / self.radioPoissonFreqDpdceT(self.nu0) ** 2
        return result
    ...
    def fgalacticDust(self, l):
      ...
      '''
        ...
        result *= self.galacticDustFreqDpdceT(self.nu1) * self.galacticDustFreqDpdceT(
            self.nu2) / self.galacticDustFreqDpdceT(self.nu0) ** 2
        return result
   ...
   def fatmosphericNoiseTT(self, l):
      ...
      return result * self.channel_corr_factor(self.nu1, self.nu2)
   '''
    ...
    def fatmosphericNoiseTT(self, l):
        ...
        def red_noise(freq):
            ...
            # ABL: interpolate freq to the SO values
            red_noise_SO = interp1d(np.array([27e9, 39e9, 93e9, 145e9, 225e9, 280e9]),
                                    np.array([200. / (2 * 1.), 77. / (2 * 1.), 1800. / (2 * 4.), 12000. / (2 * 4.),
                                              68000. / (2 * 2.),
                                              124000. / (2 * 2.)]) * A_SR / t / 0.17, bounds_error=False)
            return red_noise_SO(freq)

        # TODO: at this point, correlation of atm noise btw channels only works for hardcoded SO channels
        cc = 0
        con1 = self.nu1 == 27.3e9 and self.nu2 == 41.7e9
        con2 = self.nu2 == 27.3e9 and self.nu1 == 41.7e9
        con3 = self.nu1 == 93.e9 and self.nu2 == 143.e9
        con4 = self.nu2 == 93.e9 and self.nu1 == 143.e9
        con5 = self.nu1 == 225.e9 and self.nu2 == 278.e9
        con6 = self.nu2 == 225.e9 and self.nu1 == 278.e9
        con = con1 or con2 or con3 or con4 or con5 or con6
        if con: cc = 0.9

        if self.nu1 == self.nu2:
            result *= red_noise(self.nu1)
        else:
            result *= cc * np.sqrt(red_noise(self.nu1) * red_noise(self.nu2))
        return result
    ...
    def loadFreqDpdce(self):
        ...
        self.cmbFluctuationsFreqDpdceI = interp1d(data[:, 0], data[:, 2], kind='linear', bounds_error=False,
                                                  fill_value=0.)
        ...
        self.cmbFluctuationsFreqDpdceT = interp1d(data[:, 0], data[:, 10], kind='linear', bounds_error=False,
                                                  fill_value=0.)
        ...
        self.cibClusteredFreqDpdceT = interp1d(data[:, 0], data[:, 14], kind='linear', bounds_error=False,
                                               fill_value=0.)
        self.radioPoissonFreqDpdceT = interp1d(data[:, 0], data[:, 15], kind='linear', bounds_error=False,
                                               fill_value=0.)
        self.galacticDustFreqDpdceT = interp1d(data[:, 0], data[:, 16], kind='linear', bounds_error=False,
                                               fill_value=0.)

        # interpolate the freq dpdces for Rayleigh-Jeans temperature
        self.cmbMonopoleFreqDpdceTrj = interp1d(data[:, 0], data[:, 17], kind='linear', bounds_error=False,
                                                fill_value=0.)
        self.cmbFluctuationsFreqDpdceTrj = interp1d(data[:, 0], data[:, 18], kind='linear', bounds_error=False,
                                                    fill_value=0.)
        ...
        self.cibPoissonFreqDpdceTrj = interp1d(data[:, 0], data[:, 21], kind='linear', bounds_error=False,
                                               fill_value=0.)
        self.cibClusteredFreqDpdceTrj = interp1d(data[:, 0], data[:, 22], kind='linear', bounds_error=False,
                                                 fill_value=0.)
        self.radioPoissonFreqDpdceTrj = interp1d(data[:, 0], data[:, 23], kind='linear', bounds_error=False,
                                                 fill_value=0.)
        self.galacticDustFreqDpdceTrj = interp1d(data[:, 0], data[:, 24], kind='linear', bounds_error=False,
                                                 fill_value=0.)
    ...
    def plotFreqDpdce(self, lMin=2.e3, lMax=3.e3):
      ...
      '''
        ...
        DTcmbFluctuations /= np.sqrt(
            self.cmbFluctuationsFreqDpdceT(self.nu1) * self.cmbFluctuationsFreqDpdceT(self.nu2))
        #
...
class CIB(CMB):
    ...
    def __init__(self, beam=1., noise=1., nu1=143.e9, nu2=143.e9, lMin=30., lMaxT=3.e3, lMaxP=5.e3, name=None):
        # name
        if name is None:
            self.name = "cib_nu" + str(int(nu1 / 1.e9)) + "_nu" + str(int(nu2 / 1.e9)) + "_beam" + str(
                round(beam, 3)) + "_noise" + str(round(noise, 3)) + "_lmin" + str(int(lMin)) + "_lmaxT" + str(
                int(lMaxT)) + "_lmaxP" + str(int(lMaxP))
        else:
            self.name = name
...
class CIBPlanck15FitSimo(CMB):
    ...
    def __init__(self, beam=5., noise=1., nu1=143.e9, nu2=143.e9, lMin=30., lMaxT=3.e3, lMaxP=5.e3):
        # name
        self.name = "cibplanckfit_nu" + str(int(nu1 / 1.e9)) + "_nu" + str(int(nu2 / 1.e9)) + "_beam" + str(
            round(beam, 3)) + "_noise" + str(round(noise, 3)) + "_lmin" + str(int(lMin)) + "_lmaxT" + str(
            int(lMaxT)) + "_lmaxP" + str(int(lMaxP))
        # frequencies in Hz (irrelevant)
        self.nu1 = nu1
    ...
    def fCIBPoisson(self, l, nu1=None, nu2=None):
        ...
        if nu2 is None:
            nu2 = self.nu2
            return a_CIBP / a * (l / 3000.) ** 2 * self.mu(nu1, betaP, Td) * self.mu(nu2, betaP, Td) / self.mu(150.e9,
                                                                                                               betaP,
                                                                                                               Td) ** 2 * self.fdl_to_cl(
                l)
    ...
    def fCIBClustered(self, l, nu1=None, nu2=None):
        ...
        if nu2 is None:
            nu2 = self.nu2
            return a_CIBC * (l / 3000.) ** (2 - n) * self.mu(nu1, betaC, Td) * self.mu(nu2, betaC, Td) / self.mu(150.e9,
                                                                                                                 betaC,
                                                                                                                 Td) ** 2 * self.fdl_to_cl(
                l)
...
class CIBWuDore17(CMB):
    ...
    def __init__(self, beam=1., noise=1., nu1=143.e9, nu2=143.e9, lMin=30., lMaxT=3.e3, lMaxP=5.e3):
        # name
        #      self.name = "cmbs4"
        self.name = "cib_wudore17_nu" + str(int(nu1 / 1.e9)) + "_nu" + str(int(nu2 / 1.e9)) + "_beam" + str(
            round(beam, 3)) + "_noise" + str(round(noise, 3)) + "_lmin" + str(int(lMin)) + "_lmaxT" + str(
            int(lMaxT)) + "_lmaxP" + str(int(lMaxP))
        # frequencies in Hz
        self.nu1 = nu1
    ...
    def loadTabulatedCIB(self, nu1, nu2):
        ...
        self.flnCIB_1hln = interp1d(np.log(data[:, 0]), np.log(data[:, 1]), kind='linear', bounds_error=False,
                                    fill_value='extrapolate')
        self.fCIB_1h = lambda l: np.exp(self.flnCIB_1hln(np.log(l)))
        #
        self.flnCIB_2hln = interp1d(np.log(data[:, 0]), np.log(data[:, 2]), kind='linear', bounds_error=False,
                                    fill_value='extrapolate')
